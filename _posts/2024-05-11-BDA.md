...
L'index composite sur les colonnes `annee`, `type_statistique` et `valeur` de la table `statistiques_population` a permis d'accélérer la requête en réduisant le temps d'exécution de la requête d'environ 120 ms. Cet index est donc optimisé pour les requêtes qui filtrent sur les colonnes `annee`, `type_statistique` et `valeur` de la table `statistiques_population`, ce qui est le cas pour la requête analysée.

...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    scene.rotation.z += 0.5; // Rotate the scene slightly to view the cylinders better
    var container = document.getElementById('visualization');
    var camera = new THREE.PerspectiveCamera(60, container.clientWidth / container.clientHeight, 0.1, 1000);
    
    // Create the renderer and set its size to match the container's dimensions
    ...
    // Define blue gradient colors for the cylinders
    var colors = [
      0x6ea8fe, // Light blue
      0x4d8efb, // Medium blue
      0x356ee9  // Dark blue
    ];

    // Create the cylinders representing the database
    var cylinders = [];
    var cylinderGeometry = new THREE.CylinderGeometry(2, 2, 1, 32);
    
    for (var i = 0; i < 3; i++) {
      var material = new THREE.MeshPhongMaterial({ color: colors[i] });  // Apply the gradient
      var cylinder = new THREE.Mesh(cylinderGeometry, material);
      cylinder.position.y = i * 1.1; // Stack the cylinders
      scene.add(cylinder);
      cylinders.push(cylinder);
    }

    // Add lighting for better 3D shading effect
    var ambientLight = new THREE.AmbientLight(0xffffff, 0.8); // Soft ambient light
    scene.add(ambientLight);

    var directionalLight = new THREE.DirectionalLight(0xffffff, 0.5); // Directional light for shadows
    directionalLight.position.set(5, 5, 5);
    scene.add(directionalLight);

    // Positioning the camera to view the entire structure
    camera.position.set(0, 2, 8); // Move the camera a bit further to capture everything

    // Animation loop for rotation
    var animate = function () {
      requestAnimationFrame(animate);

      // Rotate the scene slightly on the x, y, and z axes

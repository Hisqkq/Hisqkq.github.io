...
<script>
  document.addEventListener('DOMContentLoaded', function () {
    ...
    var cylinders = [];
    var cylinderGeometry = new THREE.CylinderGeometry(2, 2, 1.3, 264, 5, false);
    
    for (var i = 0; i < 3; i++) {
      var material = new THREE.MeshPhongMaterial({ color: colors[i] });  
      var cylinder = new THREE.Mesh(cylinderGeometry, material);
      cylinder.position.y = i * 1.5; 
      scene.add(cylinder);
      cylinders.push(cylinder);
    }

    var ambientLight = new THREE.AmbientLight(0xffffff, 0.7); // Soft ambient light
